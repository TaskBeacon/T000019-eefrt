...
| Probability cues | `condition_generation.probability_levels = [0.12, 0.50, 0.88]` | `R1_TREADWAY_2009`, `R2_OHMANN_2022` | `exact` | Canonical probability levels. |
| Easy reward amount | `task.easy_reward = 1.00` | `R1_TREADWAY_2009` | `exact` | Canonical easy-option reward. |
| Hard reward range | `condition_generation.hard_reward_levels = [1.24 ... 4.30]` | `R1_TREADWAY_2009` | `exact` | Canonical hard-option reward range. |
| No-choice fallback policy | `condition_generation.no_choice_hard_prob = 0.50` | `R1_TREADWAY_2009` | `inferred` | Timeout handling is implementation-specific; deterministic preplanned fallback keeps QA/sim reproducible. |
...
| Trial count (human profile) | `task.total_trials = 48` | `R1_TREADWAY_2009`, `R2_OHMANN_2022` | `inferred` | Balanced probability x hard-reward offer grid for reproducible lab deployment. |
